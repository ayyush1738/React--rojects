...
function App() {
  ...
  async function generateAnswer(e) {
    ...
    setConversation([...conversation, newQuestionMessage]);
    ...
    try {
      ...
      setConversation([...conversation, answerMessage]);
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="fixed bottom-1 right-4">
      <button
        onClick={toggleChat}
        className="bg-transparent mt-auto mb-4 mr-4 hover:bg-blue-200 text-white py-2 px-4 rounded-full transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110"
      >
          ...
          <div className="bg-white rounded-lg shadow-md">
            ...
            <div className="h-60 overflow-y-auto px-4 py-2">
              {conversation.map((message, index) => (
                <div key={index} className={`mb-2 max-w-xs ${message.type === 'answer' ? 'text-left' : 'text-right'}`}>
                  <div className={`bg-${message.type === 'answer' ? 'gray-200' : 'blue-500'} text-${message.type === 'answer' ? 'gray-800' : 'white'} rounded-lg p-2`}>
